 ...
 * Switch User - Traverses |magic_users| to find the current user's MagicUser and the requested one.  If the requested one is a child of the current one, the current process has its UID/GID switched to that of the child.  Otherwise, the provided password will be checked */
